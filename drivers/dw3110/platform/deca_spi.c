...
#define DWM_SPI      DT_PARENT(DT_NODELABEL(spi2))
#define DWM_CS_GPIO  DT_PHANDLE_BY_IDX(DT_PARENT(DT_NODELABEL(spi2), cs_gpios, 0)
...
// int openspi(void)
// {
//     LOG_INF("%s bus %s", __func__, DT_PROP(DWM_SPI));

//     /* Propagate CS config into all spi_cfgs[] elements */
//     cs_ctrl.gpio_dev = device_get_binding(DT_PROP(DWM_CS_GPIO));
//     if (!cs_ctrl.gpio_dev) {
...
//     spi = device_get_binding(DT_PROP(DWM_SPI));
//     if (!spi) {
